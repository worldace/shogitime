...
class 棋譜{
    ...
    static 解析(text = ''){
        const 一次解析 = this.一次解析(text)

        const 先手名   = 一次解析.先手 || 一次解析.下手 || ''
        const 後手名   = 一次解析.後手 || 一次解析.上手 || ''
        const 開始手番 = this.開始手番(一次解析.開始手番, 一次解析.手合割)
        const 最終手   = this.最終手(一次解析.最終手)
        const 手合割   = 一次解析.手合割
        const 評価値   = (一次解析.解析済み) ? this.評価値(一次解析.全指し手) : []
        const 読み筋   = (一次解析.解析済み) ? this.読み筋(一次解析.全指し手) : ['-']
        const 初期局面 = {
            '駒'        : this.局面図(一次解析.局面図, 手合割),
            '先手の持駒': this.持駒(一次解析.先手の持駒 || 一次解析.下手の持駒),
            '後手の持駒': this.持駒(一次解析.後手の持駒 || 一次解析.上手の持駒),
        }
        const 全指し手 = this.全指し手(一次解析.全指し手, 開始手番)
