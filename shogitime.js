...
将棋タイム.KIF解析 = function(kif){
    ...
    解析結果.手番     = 将棋タイム.KIF解析.手番(一次解析.手番, 一次解析.手合割);
...
将棋タイム.KIF解析.手番 = function (kif手番, kif手合割){
    if(kif手番){
        return kif手番;
    }
    if(kif手合割 && kif手合割 !== "平手"){
...
将棋タイム.KIF解析.局面図 = function(kif局面図, 手合割){
    if(kif局面図.length !== 9){
        return (手合割)  ?  将棋タイム.KIF解析.局面図.手合割(手合割)  : 将棋タイム.KIF解析.局面図.初期局面();
    }

    var 局面 = 将棋タイム.KIF解析.局面図.駒無し局面();
    var 先手 = true;
    var x    = 10;
    var 変換 = {'王':'玉', '竜':'龍'};
...
将棋タイム.KIF解析.局面図.駒無し局面 = function() {
    return {
