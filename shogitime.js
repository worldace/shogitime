...
将棋タイム.KIF解析 = function(kif){
    ...
    解析結果.評価値   = (一次解析.解析済み)  ?  将棋タイム.KIF解析.評価値(一次解析.全指し手)  :  [];
    解析結果.読み筋   = (一次解析.解析済み)  ?  将棋タイム.KIF解析.読み筋(一次解析.全指し手)  :  ['-'];
    解析結果.初期局面 = {
...
将棋タイム.KIF解析.指し手 = function(kif指し手, 開始手番){
    var 全指し手 = [{手数:0, コメント:''}];
    ...
    for(var i = 0; i < kif指し手.length; i++){
        ...
        if(解析){
            ...
            全指し手.push({'手数':手数, '手番':手番, '手':'パス', '駒':'', '前X':-1, '前Y':-1, '後X':-1, '後Y':-1, '成り':false, 'コメント':''});
        }
