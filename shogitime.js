...
$s.局面.全構築 = function(){
    for(var i = 0; i < $s.指し手.length; i++){
        $s.局面.push( $s.局面.構築($s.指し手[i], $s.局面[i]) );
    }
};
...
$s.kif.解析 = function(kif){
    ...
    for(var i = 0; i < kif.length; i++){
        ...
        else if(kif[i].match(/^先手の持駒：/)){
            $s.kif.局面図の解析(局面図);
            $s.kif.持ち駒の解析(kif[i], '先手')
        }
        else if(kif[i].match(/^後手の持駒：/)){
            $s.kif.持ち駒の解析(kif[i], '後手')
        }
        else if(kif[i].match(/^\s*1/)){
...
$s.kif.指し手の解析 = function(kif){
    ...
    for(var i = 0; i < kif.length; i++){
        ...
        var 指し手 = {
            ...
            '前X' : Number(解析[5] || 0),
            '前Y' : Number(解析[6] || 0),
            '後X' : (解析[1] === '同') ? $s.指し手[i-1]['後X'] : 全数字[解析[1]],
            '後Y' : (解析[1] === '同') ? $s.指し手[i-1]['後Y'] : 漢数字[解析[2]],
            '成り': /成$/.test(解析[3]),
            '表記': ((i % 2 === 0) ? '▲' : '△') + 解析[0],
        };
