...
将棋タイム.描画.初回 = function ($b, replace){
    ...
    将棋タイム.描画($b);
    replace.parentNode.replaceChild($b.root, replace);
};
    ...
    for(var i = 0; i < kif指し手.length; i++){
        ...
        if(!解析){
            if(終局表記.indexOf(現在の手) >= 0){
                全指し手.push(将棋タイム.kif解析.勝敗結果(現在の手, 手番));
            }
            break;
        }
...
将棋タイム.kif解析.勝敗結果 = function (理由, 手番){
    var 結果 = {'勝者':'', '理由':理由, '表記':''};
