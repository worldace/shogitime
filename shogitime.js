...
$s.局面.全構築 = function(){
    for(var i = 1; i < $s.指し手.一覧.length; i++){
        $s.局面.一覧.push( 将棋タイム.局面構築($s.指し手.一覧[i], $s.局面.一覧[i-1]) );
    }
};
...
$s.kif.解析 = function(kif){
    ...
    for(var i = 0; i < kif.length; i++){
        if(kif[i].indexOf('#') === 0){
            ...
            将棋タイム.kif解析_指し手(kif.slice(i), $s.指し手.一覧);
            break;
        }
    }
    if(対局情報['先手の持駒'] && 対局情報['後手の持駒']){
        ...
        $s.局面.一覧[0]['後手持ち駒'] = 将棋タイム.kif解析_持ち駒(対局情報['後手の持駒']);
    }
    if(対局情報['先手'] && 対局情報['後手']){
        $s.先手名.名前 = 対局情報['先手'];
        $s.後手名.名前 = 対局情報['後手'];
...
将棋タイム.kif解析_持ち駒 = function(str){
    var 持ち駒 = {'歩': 0, '香': 0, '桂': 0, '銀': 0, '金': 0, '飛': 0, '角': 0};
...
将棋タイム.kif解析_指し手 = function(kif, 全指し手){
